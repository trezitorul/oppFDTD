...
using FlatTM#This allows for access to the simulator

function sineSource(i,j,k,t,FDS,BC)#BC = (I,w, normal)S
	modT=2
	FDS[i,j,k,modT,3]=BC[1]*sin(BC[2]*t)
	#FDS[i,j,k,modT,1]=BC[1]*sin(BC[2]*t)
end

function createYee2DOperator(OppBC, MatBC)
	OH=FlatTM.operator((FlatTM.HStep), MatBC)
	OE=FlatTM.operator((FlatTM.EStep), MatBC)
	I=FlatTM.operator((FlatTM.Identity),0)
	Wall=FlatTM.operator((FlatTM.EMWall), PECWallBC)
	sineSrc=FlatTM.operator((sineSource), (1,srcFreq,3))#BC for this source 
	return FlatTM.operator((I,OE, OH, Wall, sineSrc),OppBC)
end
...
simTimeSteps = 100000
#println(FDS)
FlatTM.simulate(tSteps, sampleRate, FDS, simulator)
